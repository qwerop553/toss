...
class DB:
    ...
    def load(self, ticker: str, start = None, end = None, limit = 1000):
        conn = sqlite3.connect(self._DB_PATH)
        query = "SELECT * FROM (SELECT timestamp, open, high, low, close, volume FROM candles where ticker = ?"
        params = [ticker]
...
if __name__ == "__main__":
    db = DB()
    print(db.update("005930", "000660"))
    print(db.load("005930", limit=100))
