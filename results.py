...
def _candles(ticker: str, interval: str) -> pd.DataFrame:
    ...
    """
    return scrap.load_candles(ticker, interval).reset_index(drop=True)
...
def _fingerprint(name: str) -> float:
    ...
    files = [
        ...
        os.path.join(BASE, "strategies", "indicators.py"),
        os.path.join(BASE, "strategies", "base.py"),
        os.path.join(BASE, "backtest_engine.py"),
    ]
